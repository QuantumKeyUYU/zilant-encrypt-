"""Desktop GUI for Zilant Encrypt (Modern & Robust)."""
...
# --- Styles & Constants (Theme: "Modern Zinc") -----------------------

BG_MAIN = "#18181B"        # Main Background (Dark Zinc)
BG_SURFACE = "#27272A"     # Cards / Panels
BG_INPUT = "#3F3F46"       # Input Fields
...
QRadioButton::indicator:checked {{
    border: 5px solid {BG_SURFACE};
    background-color: {ACCENT};
}}

/* --- Labels --- */
QLabel {{ color: {TEXT_PRIMARY}; }}
QLabel#H1 {{ font-size: 20px; font-weight: 800; letter-spacing: 0.5px; }}
QLabel#Subtitle {{ color: {TEXT_SECONDARY}; font-size: 13px; margin-bottom: 5px; }}
QLabel#H2 {{ font-size: 14px; font-weight: 700; margin-bottom: 6px; color: {TEXT_PRIMARY}; }}
QLabel#StatusIcon {{ font-size: 14px; }}
...
if QT_AVAILABLE:
    ...
    class ZilantWindow(QtWidgets.QMainWindow):
        ...
        def _build_encrypt_tab(self) -> None:
            ...
            radio_style = "font-weight: 700; font-size: 15px; padding: 5px;"
            self.encrypt_radio.setStyleSheet(radio_style)
            self.decrypt_radio.setStyleSheet(radio_style)
            ...
            io_vbox.setSpacing(16)

            # Source Type
            type_layout = QtWidgets.QHBoxLayout()
            self.type_label = QtWidgets.QLabel(self.ui_strings.source_type)
            self.type_label.setStyleSheet(f"color: {TEXT_SECONDARY}; font-weight: 600;")
            ...
            # Using Unicode icon for visual cue
            self.input_label.setText(f"📄 {self.ui_strings.input_path}")
            ...
            self.output_label.setText(f"💾 {self.ui_strings.output_path}")
            ...
            self.pass_label.setText(f"🔒 {self.ui_strings.password_ph}")
            ...
            self.decoy_password_edit.setMinimumHeight(42)
            ...
            self.action_button.setMinimumHeight(55)
        ...
        def _build_inspect_tab(self) -> None:
            ...
            self.inspect_pass_edit.setMinimumHeight(42)
        ...
        def _build_footer(self) -> None:
            ...
            # Button hidden initially, shown only on success
            self.open_folder_btn = QtWidgets.QPushButton(f"📂 {self.ui_strings.open_folder}")
        ...
        def _retranslate_ui(self) -> None:
            ...
            self.open_folder_btn.setText(f"📂 {self.ui_strings.open_folder}")
        ...
        def _browse_output(self) -> None:
            # We allow picking either a file or a folder
        ...
        def _on_action_clicked(self) -> None:
            ...
            raw_out_path = self.output_edit["edit"].text().strip() or self.output_edit["edit"].placeholderText()
            ...
            # --- SMART PATH LOGIC (Fixing WinError 5) ---
            try:
                out_path_obj = Path(raw_out_path)
                in_path_obj = Path(in_path)

                # If the user selected a directory as output, we auto-append the filename
                if out_path_obj.is_dir():
                    if is_encrypt:
                        # Encrypt: /folder/input.txt -> /out_dir/input.txt.zil
                        file_name = in_path_obj.name + ".zil"
                        out_path_obj = out_path_obj / file_name
                    else:
                        # Decrypt: /folder/archive.zil -> /out_dir/archive
                        # If extension is .zil, strip it. Otherwise append .decrypted
                        if in_path_obj.suffix.lower() == ".zil":
                            file_name = in_path_obj.stem
                        else:
                            file_name = in_path_obj.name + ".decrypted"
                        out_path_obj = out_path_obj / file_name

                out_path = str(out_path_obj)
            except Exception:
                # Fallback to raw string if path logic fails
        ...
        def _on_worker_finished(self, success: bool, message: str) -> None:
            ...
            if success:
                ...
                # Do not clear passwords on error, so user can correct typos
        ...
        def _open_output_folder(self) -> None:
            if self._output_path:
                ...
                # If target is file, open parent folder. If directory, open itself.
    ...
    def create_app() -> Any:
        ...
        # Set a global fusion style as a base for standard widgets
        app.setStyle("Fusion")
        window = ZilantWindow()
        window.show()
