"""Desktop GUI for Zilant Encrypt with Modern Zinc theme."""
...
def _format_overview_report(
    path: Path, overview: ContainerOverview, validated: list[int], pq_available: bool, strings: Strings
) -> str:
...
if QT_AVAILABLE:
    ...
    class TaskWorker(QtCore.QThread):
        ...
        def run(self) -> None:
            try:
                ...
                self.finished_error.emit(self._strings.processing_integrity_error.format(error=e))
            except Exception as e:
                traceback.print_exc()
                self.finished_error.emit(self._strings.processing_unexpected_error.format(error=e))
    ...
    class ZilantWindow(QtWidgets.QMainWindow):
        ...
        def _build_encrypt_tab(self) -> None:
            ...
            vbox.addWidget(self.action_button, alignment=QtCore.Qt.AlignmentFlag.AlignRight)
        ...
        def _build_inspect_tab(self) -> None:
            ...
            insp_layout.addWidget(QtWidgets.QLabel("📄 " + self.ui_strings.inspect_select_ph))
            ...
            insp_layout.addWidget(self.inspect_btn, alignment=QtCore.Qt.AlignmentFlag.AlignRight)
        ...
        def _suggest_output_name(self, input_path: Path, is_encrypt: bool) -> str:
            ...
            if input_path.name.endswith(".zil"):
                return input_path.name[: -len(".zil")]
            return input_path.name + ".decrypted"

        def _suggest_output_path(self, input_path: Path, is_encrypt: bool) -> Path:
            return input_path.with_name(self._suggest_output_name(input_path, is_encrypt))

        def _resolve_output_path(self, input_path: Path, out_text: str | None, is_encrypt: bool) -> Path:
            base_name = self._suggest_output_name(input_path, is_encrypt)
            if out_text:
                candidate = Path(out_text)
                if candidate.exists() and candidate.is_dir():
                    return candidate / base_name
                if not candidate.suffix and out_text.endswith(("/", "\\")):
                    return candidate / base_name
                return candidate
        ...
        def _update_defaults(self) -> None:
            ...
            target = self._suggest_output_path(in_path, self.encrypt_radio.isChecked())
        ...
        def _on_action_clicked(self) -> None:
            ...
            resolved_out = self._resolve_output_path(Path(in_path_str), out_raw or out_placeholder, is_encrypt)
            ...
            def enc_task() -> None:
                if self.decoy_group.isChecked():
                    ...
                    encrypt_file(Path(in_path_str), resolved_out, password, mode=mode, overwrite=overwrite)
            ...
            def dec_task() -> None:
                ...
                if vol:
                    ...
                    if _DECRYPT_AUTO_SUPPORTS_OVERWRITE:
                        ...
                        decrypt_auto_volume(
                            Path(in_path_str), Path(resolved_out), password=password, mode=mode
                        )
        ...
        def _show_about_dialog(self) -> None:
            QtWidgets.QMessageBox.about(
                self, self.ui_strings.about_title, self.ui_strings.about_body.format(version=__version__)
            )
